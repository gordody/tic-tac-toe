...
export class Board<ValueType> implements IBoard<ValueType> {
  ...
  getAt(x: number, y: number): ValueType {
    if (x < 0 || x >= this.width || y < 0 || y >= this.height) {
      throw new Error(
        `Error getting cell: board index out of bounds: x: ${x} - width: ${this.width}, y: ${y} - height: ${this.height}`
      );
    }
  ...
  setAt(x: number, y: number, value: ValueType): void {
    ...
    if (x < 0 || x >= this.width || y < 0 || y >= this.height) {
      throw new Error(
        `Error setting cell: board index out of bounds: x: ${x} - width: ${this.width}, y: ${y} - height: ${this.height}`
      );
    }
